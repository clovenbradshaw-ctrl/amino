...
import type { Token, TokenType, FormulaAST } from './types';
...
class Parser {
  ...
  private parsePrimary(): FormulaAST {
    ...
    if (tok.type === 'FIELD_REF') {
      this.advance();
      return { type: 'field_ref', name: tok.value as string };
    }

    // String literal
    if (tok.type === 'STRING') {
      this.advance();
      return { type: 'literal', value: tok.value as string, dataType: 'string' };
    }

    // Number literal
    if (tok.type === 'NUMBER') {
      this.advance();
      return { type: 'literal', value: tok.value as number, dataType: 'number' };
    }
 ...
 */
export function parseAirtableFormula(formula: string): FormulaAST {
