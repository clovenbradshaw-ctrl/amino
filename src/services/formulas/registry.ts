...
import { parseAirtableFormula, collectFieldRefs } from './parser';
import { translateToEoIR, translateLookupToEoIR, translateRollupToEoIR } from './eo-ir';
import { compileFormula } from './compiler';
import { compileLookup, compileRollup } from './relational-compiler';
import type {
  CompiledField,
  FieldRegistryEntry,
  DataContext,
...
export class FormulaRegistry {
  private fieldEntries: FieldRegistryEntry[];
  private dataContext: DataContext;
  private tableId: string;
  private baseId: string;

  /** Compiled fields indexed by field ID */
  private compiled: Map<string, CompiledField>;

  /** Field IDs in dependency order (leaves first) */
  private dependencyOrder: string[];

  constructor(
    fieldEntries: FieldRegistryEntry[],
  ...
  compile(): void {
    ...
    for (const fieldId of this.dependencyOrder) {
      ...
      try {
        ...
        console.error(`Failed to compile ${entry.fieldName}: ${(e as Error).message}`);
        // Register a stub that returns the error
        this.compiled.set(fieldId, {
          fieldId: entry.fieldId,
          fieldName: entry.fieldName,
          fieldType: entry.fieldType,
          execute: () => ({ __error: true, message: `Compile error: ${(e as Error).message}` }),
          eoIR: null,
          sourceFieldNames: [],
   ...
   */
  private _compileField(entry: FieldRegistryEntry): void {
    ...
    if (entry.fieldType === 'formula') {
      ...
      const fieldRegistry = new Map(
        this.fieldEntries.map(f => [f.fieldName, f] as [string, FieldRegistryEntry])
      );
      ...
      const eoIR = translateLookupToEoIR(
        ...
        linkedTableId,
        (options.result || { type: 'string' }) as ResultType,
        ctx
      );
      ...
      this.compiled.set(entry.fieldId, {
        ...
        fieldName: entry.fieldName,
        fieldType: 'lookup',
        execute: (record: Record<string, unknown>) => executeFn(record),
        eoIR,
      ...
      const foreignFieldName = this._resolveFieldName(options.fieldIdInLinkedTable || '', linkedTableId);
      const linkFieldName = this._resolveFieldName(options.recordLinkFieldId || '', this.tableId);
      const aggregation = (options.formula || 'ARRAYJOIN(values)') as string;
   ...
   */
  getCompiledField(fieldId: string): CompiledField | undefined {
   ...
   */
  getCompiledFieldByName(name: string): CompiledField | undefined {
   ...
   */
  private _resolveLinkedTable(linkFieldId: string): string {
   ...
   */
  private _resolveFieldName(fieldId: string, tableId: string): string {
   ...
   */
  private _buildDependencyGraph(computedFields: FieldRegistryEntry[]): Map<string, Set<string>> {
   ...
   */
  private _topologicalSort(deps: Map<string, Set<string>>): string[] {
    const sorted: string[] = [];
